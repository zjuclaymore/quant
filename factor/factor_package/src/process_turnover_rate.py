...
处理链:
    ...
    3. 行业 + 市值中性化
    4. Z-Score 截面标准化
输出: factor_package/turnover_rate_processed_parquet/<YYYYMMDD>.parquet
"""
...
def batch_process():
    ...
    raw_dir    = config['paths']['turnover_rate_raw']
    output_dir = config['paths']['turnover_rate_processed']
    os.makedirs(output_dir, exist_ok=True)

    # 写 _meta.json
    meta = {
        'factor_name': 'turnover_rate',
        'description': 'Processed turnover_rate factor (Winsorized, Neutralized, Standardized)',
        'processing_steps': [
    ...
    for filename in files:
        ...
        try:
            ...
            factor_name_in_file = 'turnover_rate_20d'
            ...
            if not processed_series.empty:
                ...
                processed_df.columns = ['symbol', 'turnover_rate']
