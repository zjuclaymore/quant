...
处理链:
    ...
    3. 行业 + 市值中性化
    4. Z-Score 截面标准化
输出: factor_package/1_minus_r2_30_processed_parquet/<YYYYMMDD>.parquet
"""
...
def batch_process():
    ...
    raw_dir    = config['paths']['1_minus_r2_30_raw']
    output_dir = config['paths']['1_minus_r2_30_processed']
    os.makedirs(output_dir, exist_ok=True)

    # 写 _meta.json
    meta = {
        'factor_name': '1_minus_r2_30',
        'description': 'Processed 1_minus_r2_30 factor (Winsorized, Neutralized, Standardized)',
        'processing_steps': [
    ...
    for filename in files:
        ...
        try:
            ...
            factor_name_in_file = '1_minus_r2'
            if factor_name_in_file not in df.columns:
                # 降级尝试，如果只有一个非 symbol 列
                cols = [c for c in df.columns if c != 'symbol']
                if cols:
                    factor_name_in_file = cols[0]
                else:
                    continue

            factor_series = df.set_index('symbol')[factor_name_in_file]
            ...
            if not processed_series.empty:
                ...
                processed_df.columns = ['symbol', '1_minus_r2_30']
