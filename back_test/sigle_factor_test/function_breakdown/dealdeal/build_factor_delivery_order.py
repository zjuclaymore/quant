...
def build_delivery_order(
    ...
    min_date = pd.to_datetime(factor["date"].min())
    max_date = pd.to_datetime(factor["date"].max())
    start_date = min_date.strftime("%Y-%m-%d")
    end_date = max_date.strftime("%Y-%m-%d")

    t = time.perf_counter()
    cal = _load_calendar_df(calendar_py_path, start_date, end_date, buyday, sellday)
    logger.info("[Order] 调仓日历完成: 期数=%d, 耗时=%.2fs", len(cal), time.perf_counter() - t)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--buyday", type=str, default="month_start", help="买入日规则")
