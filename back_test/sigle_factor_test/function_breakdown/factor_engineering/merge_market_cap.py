...
def load_market_cap_data(mv_path=None):
    ...
    if 'lncap' not in df_mv.columns:
        # 尝试找到市值列
        potential_cols = [c for c in df_mv.columns if 'mv' in c.lower() or 'cap' in c.lower() or 'log' in c.lower()]
        if potential_cols:
    ...
    df_mv = df_mv[['code', 'date', 'lncap']].drop_duplicates().sort_values(['date', 'code']).reset_index(drop=True)
    
    print(f"[Market Cap Loader] ✓ 加载完成: {len(df_mv)} 行")
    print(f"[Market Cap Loader] 日期范围: {df_mv['date'].min()} ~ {df_mv['date'].max()}")
    print(f"[Market Cap Loader] 股票数: {df_mv['code'].nunique()}")
    
    return df_mv


def standardize_market_cap_columns(df):
    """
    标准化市值数据的列名
...
def merge_market_cap_to_factor(df_factor, df_mv, on=['code', 'date'], how='left'):
    ...
    print(f"[Merger] 市值列缺失率: {(df_merged['lncap'].isna().sum() / len(df_merged) * 100):.2f}%")
