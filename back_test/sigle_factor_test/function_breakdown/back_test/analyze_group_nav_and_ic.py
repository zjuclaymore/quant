...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="按时间区间计算分组净值曲线与 IC/RANKIC/IR/RANKIR")
...
def main() -> int:
    ...
    df[args.group_col] = pd.to_numeric(df[args.group_col], errors="coerce").astype("Int64")
    ...
    # IC/IR 使用全市场个股口径（不做分组截断）。
    df_ic_base = df.copy()

    # 分组净值可选按每组股票数做截断，专门用于单调性评估。
    if args.max_stocks_per_group and args.max_stocks_per_group > 0:
        tie_cols = [c for c in ["stock_code", "code", "wind_code", "S_INFO_WINDCODE"] if c in df.columns]
    ...
    grp_ret = (
        grp.groupby([args.date_col, args.group_col], as_index=False)
        .agg(
    ...
    nav = grp_ret.pivot(index=args.date_col, columns=args.group_col, values="group_return").sort_index()
    nav = nav.fillna(0.0)
    nav = (1.0 + nav).cumprod()
    ...
    # 额外输出：每组因子均值随时间变化
    factor_mean = grp_ret.pivot(index=args.date_col, columns=args.group_col, values="group_factor_mean").sort_index()
    ...
    # 2) 每期 IC / RANKIC（横截面: stock_factor vs stock_return，全市场个股口径）
    ic_rows = []
    for d, g in df_ic_base.groupby(args.date_col):
