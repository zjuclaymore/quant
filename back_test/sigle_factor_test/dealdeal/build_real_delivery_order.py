...
def main() -> int:
    ...
    use_cols = [c for c in ["date", "code", "allow_flag", "volume", "收盘价(元)", "涨停价(元)", "跌停价(元)"] if c in pool_cols]
    ...
    # 不再在此处过滤 can_buy，将各方案独立的交易逻辑交给下游 enrichment 环节。
    # 只合并基础信息。
    real_order = paper.merge(mask[["buy_int", "code", "allow_flag", "tradable_base"]], on=["buy_int", "code"], how="left")
