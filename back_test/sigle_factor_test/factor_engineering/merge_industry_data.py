...
def load_industry_data(ind_path=None):
    ...
    # 默认产业路径
    if ind_path is None:
        ind_path = r"E:\1_basement\quant_research\data\申万行业分类2021版_AShareSWNIndustriesClass"
    ...
    # 尝试加载 Pickle 文件（优先）
    pickle_files = glob.glob(os.path.join(ind_path, "*.pickle")) + glob.glob(
        os.path.join(ind_path, "*.pkl")
    )
    if pickle_files:
    ...
    # 优先使用 Parquet 文件（标准化后的合并文件）
    parquet_files = glob.glob(os.path.join(ind_path, "*.parquet"))
    if parquet_files:
        # 优先使用 industry_for_merge.parquet
        preferred = [f for f in parquet_files if "industry_for_merge" in f]
        if preferred:
            try:
                df_ind = pd.read_parquet(preferred[0])
                print(
                    f"[Industry Loader] 从Parquet加载: {os.path.basename(preferred[0])}"
                )
            except Exception as e:
    ...
    # 如果没有 Parquet，尝试 Pickle 文件（已在上方尝试过）
    ...
    # 如果没有行业名称列，只要行业代码列即可
    if len(keep_cols) < 2:
        if "ind_code" in df_ind.columns or "SW_IND_CODE" in df_ind.columns:
            keep_cols = [c for c in ["code", "ind_code"] if c in df_ind.columns]
        else:
            raise ValueError(f"无法找到产业代码列。可用列: {list(df_ind.columns)}")

    # 标准化股票代码格式
    if "code" in df_ind.columns:
        df_ind["code"] = df_ind["code"].astype(str).apply(_standardize_code)

    # 去重和排序
    df_ind = (
        df_ind[keep_cols].drop_duplicates().sort_values("code").reset_index(drop=True)
    )
    ...
    print(
        f"[Industry Loader] 产业数: {df_ind['ind_code'].nunique() if 'ind_code' in df_ind.columns else '?'}"
    )
...
def standardize_industry_columns(df):
    ...
    # 标准化 industry code 列 - 增强对 SW_IND_CODE 的处理
    if "SW_IND_CODE" in df.columns:
        if "ind_code" not in df.columns:
            df = df.rename(columns={"SW_IND_CODE": "ind_code"})
...
def main():
    ...
    try:
        ...
        df_merged = merge_industry_to_factor(
            df_factor, df_ind, on="code", how=args.merge_method
        )
